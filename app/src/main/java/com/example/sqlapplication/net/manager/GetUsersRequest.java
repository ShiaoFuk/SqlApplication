...
public class GetUsersRequest {
    private final static String TAG = GetUsersRequest.class.getName();

    public static void get(RequestListener listener, MyRequestBody<UserPermission> requestBody) {
        listener.onRequest();
    ...
    private static JsonElement request(MyRequestBody<UserPermission> requestBody) throws IOException {
        OkHttpClient client = new OkHttpClient().newBuilder()
                .build();
        MediaType mediaType = MediaType.parse("application/json");
        RequestBody body = RequestBody.create(mediaType, JsonUtils.toJson(requestBody));
        Request request = new Request.Builder()
                .url("http://suansun.top/user/getUser")
                .method("POST", body)
