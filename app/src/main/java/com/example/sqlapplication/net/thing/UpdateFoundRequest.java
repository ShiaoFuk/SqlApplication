...
public class UpdateFoundRequest {
    private final static String TAG = UpdateFoundRequest.class.getName();

    public static void get(RequestListener listener, MyRequestBody<Thing> requestBody) {
        listener.onRequest();
    ...
    private static JsonElement request(MyRequestBody<Thing> requestBody) throws IOException {
        OkHttpClient client = new OkHttpClient().newBuilder()
                .build();
        MediaType mediaType = MediaType.parse("application/json");
        RequestBody body = RequestBody.create(mediaType, JsonUtils.toJson(requestBody));
        Request request = new Request.Builder()
                .url("http://suansun.top/thing/found/update")
                .method("POST", body)
