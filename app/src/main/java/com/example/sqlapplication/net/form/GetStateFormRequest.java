...
public class GetStateFormRequest {
    private final static String TAG = GetStateFormRequest.class.getName();

    public static void get(RequestListener listener, MyRequestBody<FormState> myRequestBody) {
        listener.onRequest();
    ...
    private static JsonElement request(MyRequestBody<FormState> myRequestBody) throws IOException {
        OkHttpClient client = new OkHttpClient().newBuilder()
                .build();
        MediaType mediaType = MediaType.parse("application/json");
        RequestBody body = RequestBody.create(mediaType, JsonUtils.toJson(myRequestBody));
        Request request = new Request.Builder()
                .url("http://suansun.top/form/get")
                .method("POST", body)
